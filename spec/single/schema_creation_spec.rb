...
RSpec.describe 'Schema creation', :migrations do
  ...
  describe 'add column' do
    it 'adds the specified column at the end of the table' do
      ...
      expect(model.columns.map(&:name).last).to eq('location')
    end
    ...
    context 'with :after option' do
      it 'adds the specified column in the correct order' do
        ...
        col_names = model.columns.map(&:name)
        event_name_index = col_names.index('event_name')
        expect(col_names.index('location')).to eq(event_name_index + 1)
      end
    end
